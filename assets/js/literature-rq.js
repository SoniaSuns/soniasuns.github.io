(function () {
  ...
  function discoveryLabel(p) {
    if (p.discovery_method === "author_search") return "作者署名检索 · " + p.seed;
    if (p.discovery_method === "context_search") return "选题定位补充 · " + p.seed;
    return "发现自 " + p.seed + " · 第 " + p.depth + " 轮引用/线索检索";
  }
  ...
  function renderSummary() {
    ...
    document.getElementById("rq-route-summary").innerHTML = '<div class="rq-root"><span class="rq-id">' + r.root + ' · 长期母问题 · 文献组织框架</span><h3>' + esc(r.short_title || r.label) + '</h3><p>' + esc(r.explanation) + '</p><details><summary>展开完整母问题与评价维度</summary><p>' + esc(r.question) + '</p><p><strong>怎样判断进展：</strong>' + esc(r.metrics) + '</p>' + sourceButtons(r.anchors) + '</details></div>';
  ...
  function renderDetail() {
    ...
    if (p && p.annotation) {
      var annotationBlock = document.createElement('div');
      annotationBlock.className = 'rq-annotation-link';
      var reviewLabels = { method_checked: '方法条件已核查', abstract_review: '摘要初读', existing_notes: '既有阅读笔记', metadata_only: '仅书目 / 待补读' };
      annotationBlock.innerHTML = '<p><a href="#evidence-' + esc(p.id) + '">查看逐项输入、训练与推理条件 ↑</a></p><p class="rq-muted">阅读深度：' + esc(reviewLabels[p.annotation.review_status] || '待补读') + ' · 结果未独立复现</p>';
      detail.insertBefore(annotationBlock, detail.querySelector('h4'));
    }
